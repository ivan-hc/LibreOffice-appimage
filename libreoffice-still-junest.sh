...
cp -r archlinux/.junest/lib/libreoffice/share/xdg/*startcenter* "$APP".AppDir/
ICON=$(cat "$APP".AppDir/*.desktop | grep "Icon=" | cut -c 6-)
[ -z "$ICON" ] && ICON="$BIN"
...
VERSION="$VERSION"
UPINFO="gh-releases-zsync|$GITHUB_REPOSITORY_OWNER|$REPO|$TAG|*still*x86_64.AppImage.zsync"

ARCH=x86_64 ./appimagetool --comp zstd --mksquashfs-opt -Xcompression-level --mksquashfs-opt 20 \
	-u "$UPINFO" \
	./"$APP".AppDir "$APPNAME"-still_"$VERSION"-archimage4.3-x86_64.AppImage
